...
In ChannelMerger:
	...
	1.	PositionGetter is used to generate and retrieve 2D positions for each channel in the MEG data.
	2.	These positions are then passed through FourierEmb, which transforms the 2D positional coordinates into high-dimensional Fourier embeddings.
	3.	The resulting embeddings are used by the model to combine or transform the MEG channels in a position-aware manner, allowing the model to capture spatial relationships between different channels in the data.
...
class SubjectLayers(nn.Module):
    ...
    def forward(self, x, subjects):
        ...
        # print("subjects device", subjects.device)
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, block_number, dilation_type, extra_dropout):
        super().__init__()
        k = block_number # layer index for calculating dilation
        
        if dilation_type == "cyclic":
            # Layer 1 has dilation 2 ^ (2k mod 5)
            print("Dilation type is cyclic")
            l1_dilation = 2 ** ((2 * k) % 5)
            # Layer 2 has dilation 2 ^ ((2k + 1) mod 5)
            l2_dilation = 2 ** ((2 * k + 1) % 5)
            l3_dilation = 2
        elif dilation_type == "expo":
            print("Dilation type is exponentially increasing")
            if k == 1:
                l1_dilation = 1
                l2_dilation = 2
                l3_dilation = 4
        ...
        print("l1_dilation", l1_dilation)
        print("l2_dilation", l2_dilation)
        print("l3_dilation", l3_dilation)
        
        self.conv1 = nn.Conv1d(in_channels, out_channels, kernel_size=3, stride=1, dilation= l1_dilation, padding='same') # we don't want the time signal length to change
        self.act1 = nn.GELU()
        # dilation = dilation * 2
        self.conv2 = nn.Conv1d(out_channels, out_channels, kernel_size=3, stride=1, dilation=l2_dilation, padding='same')
        self.act2 = nn.GELU()
        # dilation = dilation * 2
        self.conv3 = nn.Conv1d(out_channels, 2 * out_channels, kernel_size=3, stride=1, dilation=l3_dilation, padding='same')
    ...
    def forward(self, x):
        residual1 = self.skip(x)
        x = self.act1((self.norm1(self.conv1(x)))) # Remove the dropout layer
        if x.shape == residual1.shape:
            x = x + residual1
        residual2 = self.skip(x)
        x = self.act2(self.dropout(self.norm2(self.conv2(x))))
        if x.shape == residual2.shape:
            x = x + residual2
        residual3 = self.skip(x)
        x = self.glu(self.conv3(x))
        if x.shape == residual3.shape:
            x = x + residual3
        x = self.dropout(x)
        return x
...
class PositionGetter:
    ...
    def get_recording_layout(self, recording) -> torch.Tensor:
        # recording is changed to be a epochs object
        index = 0 #recording.recording_index
        if index in self._cache:
            ...
            for meg_index, name in enumerate(info.ch_names):
                name = name.rsplit("-", 1)[0] #split from right, return everything before the last occurrence of '-'
                try:
            ...
            positions = torch.full((len(info.ch_names), 2), self.INVALID) # here invalid is set to =0.1.returns a len(info.ch_names) by 2 matrix filled with -0.1
    ...
    def get_positions(self, batch):
        ...
        for idx in range(len(batch)):
            ...
            rec_pos = self.get_recording_layout(meg)
...
class FourierEmb(nn.Module):
    ...
    """
    def __init__(self, dimension: int = 272, margin: float = 0.1):
        super().__init__()
...
class ChannelDropout(nn.Module):
    ...
    def forward(self, meg, batch, ch_pos):
        if not self.dropout:
            return meg
...
class ChannelMerger(nn.Module):
    def __init__(self, chout: int, pos_dim: int = 256,
                 ...
                 n_subjects: int = 4, per_subject: bool = False):
        super().__init__()
        assert pos_dim % 4 == 0
        self.position_getter = PositionGetter()
    ...
    def forward(self, meg, batch, ch_pos):
        B, C, T = meg.shape
        # print("meg device", meg.device)
        meg = meg.clone()
        # positions = self.position_getter.get_positions(batch)
        positions = ch_pos
        # print("meg device", meg.device)
        embedding = self.embedding(positions).to(meg.device)
        ...
        # print("embedding inside channel merger device", embedding.device)
        # print("heads inside channel merger device", heads.device)
